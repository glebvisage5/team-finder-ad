...
Создайте файл `.env` в корне проекта вручную (не копируйте из ZIP — возможны проблемы с кодировкой на Windows).

Содержимое файла (только ASCII-символы, без кириллицы и спецсимволов):

```env
DJANGO_SECRET_KEY=замените-на-сгенерированный-ключ
DJANGO_DEBUG=True
...
Перед миграциями убедитесь, что контейнер с PostgreSQL запущен:
...
В списке должен быть `teamfinder_db` со статусом `Up`. Только после этого:
